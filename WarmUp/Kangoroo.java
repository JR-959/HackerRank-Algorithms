...
public class Kangoroo {
	...
	static String kangaroo(int x1, int v1, int x2, int v2) {
		/*
		 * Since we want two kangaroos to meet at a certain jump, we can make an
		 * equation as following: x1+ jump*v1 = x2+jump*v2 Further simplification leads
		 * to jump*(v2-v1) = (x2-x1) And finally, jump = (x2-x1)/(v1-v2)
		 */
		if (v1 > v2) {
			double jumps = (x2 - x1) / (1.0 * (v1 - v2)); // multiplication in denominator (1.0) is needed to make sure
															// that our result for jump is double
